 ...
 * @return array
 */
function otgts2_validate_email ($in) {
	if (empty($in))
 ...
 * @return boolean
 */
function otgts2_verify_hcaptcha () {
	$post_data['secret'] = get_option('otgts2_hcaptcha_secret_key');
	if (empty($post_data['secret'])) return true; // hcaptcha is disabled
	$post_data['response'] = otgts2_get_request_string('h-captcha-response');
	...
	if (!empty($curl_error)) {
		error_log('otgts2_verify_hcaptcha Curl Error: ' . print_r($curl_error, true));
	}
	$curl_info = curl_getinfo($curl);
	error_log('otgts2_verify_hcaptcha Curl Info: ' . print_r($curl_info, true));
	error_log('otgts2_verify_hcaptcha Response: ' . $response);
	error_log('otgts2_verify_hcaptcha Response Array: ' . print_r($response_data, true));
	/**/
